...
public class LB_LightingBoxHelper : MonoBehaviour {
	...
	void UpdatePostEffects(LB_LightingProfile profile)
	{
		...
		helper.Update_Bloom(profile.Bloom_Enabled,profile.bIntensity,profile.bThreshould,profile.bColor,profile.dirtTexture,profile.dirtIntensity,profile.mobileOptimizedBloom);
	...
	public void Update_ChromaticAberration(bool enabled,float intensity,bool mobileOptimized)
	{
		...
		ca.mobileOptimized.overrideState = true;
		ca.mobileOptimized.value = mobileOptimized;

		ca.enabled.value = enabled; 
	}

	public void Update_Bloom(bool enabled,float intensity,float threshold,Color color,Texture2D dirtTexture,float dirtIntensity,bool mobileOptimized)
	{
		if(enabled)
		{
			Bloom b;
			...
			b.mobileOptimized.overrideState = true;
			b.mobileOptimized.value = mobileOptimized;
